...
```{r transect_map, message = FALSE, warning = FALSE, echo = FALSE, fig.cap = "\\label{fig:transect_map}Wisconsin Spring Duck Survey aerial transects labeled by transect number and survey region. The four regions surveyed are the Northern High Density region (NHI), Northern Low Density region (NLO), Southeast Central region (SEC), and Southwest Driftless region (SWD)."}
plot_transect_map()
```

\newpage

```{r wetland_abundance, message = FALSE, warning = FALSE, echo = FALSE, fig.cap =  paste0("\\label{fig:wetland_abundance}", "Annual variability in total non-linear and linear wetlands per square mile by survey region.")}
...
```{r annual_crane_counts, message = FALSE, warning = FALSE, echo = FALSE, fig.cap = paste0("\\label{fig:annual_crane_counts}", "Annual counts of sandhill cranes observed from the air during the Waterfowl Breeding Population Survey, 1996--", survey_year, ".")}
plot_crane_counts()
```
