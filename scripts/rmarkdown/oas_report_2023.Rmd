...
title: |
  ...
  | **Waterfowl Breeding Population Survey For Wisconsin, 1973\textemdash`r params$year`**
  |
author:  |
  | Amy Shipley^1^, Jason Winiarski^1^, and Taylor Finger^2^
...
header-includes:
  ...
  - \usepackage{titlesec}
  - \usepackage{titling}
  - \pretitle{\centering\Huge\fontspec{Anton}}
  - \posttitle{\vspace{0.5 cm}\centering\Huge\fontspec{Anton}}
  - \usepackage{sectsty}
  - \sectionfont{\LARGE\fontspec{Anton}}
...
previous_year <- 
  read_csv(str_c(here::here('output'), '/', analysis_year, '/', 'ws_structured.csv'), show_col_types = FALSE) %>%
  distinct(year) %>%
  slice_tail(n = 2) %>%
  slice(1) %>%
  pull()
...
sec_results <- 
  read_csv(
    str_c(here::here('output'), '/', analysis_year, '/', 'wetland_summary_region_1.csv'), 
    show_col_types = FALSE
  )

nhi_results <- 
  read_csv(
    str_c(here::here('output'), '/', analysis_year, '/', 'wetland_summary_region_2.csv'), 
    show_col_types = FALSE
  )

nlo_results <- 
  read_csv(
    str_c(here::here('output'), '/', analysis_year, '/', 'wetland_summary_region_3.csv'), 
    show_col_types = FALSE
  )

swd_results <- 
  read_csv(
    str_c(here::here('output'), '/', analysis_year, '/', 'wetland_summary_region_4.csv'), 
    show_col_types = FALSE
  )
```
...
comp_other_ducks <- 
  read_csv(
    str_c(here::here('output'), '/', analysis_year, '/', 'comp_oducks_state', "_", analysis_year, '.csv'),
    show_col_types = FALSE
  )
...
air_coot_count <- 
  read_csv(
    str_c(here::here('output'), '/', analysis_year, '/', 'coots', "_", analysis_year, '.csv'), 
    show_col_types = FALSE
  ) %>% 
  slice_tail(n= 1) %>% 
  pull(coots)
...
air_crane_count <- 
  read_csv(
    str_c(here::here('output'), '/', analysis_year, '/', 'cranes', "_", analysis_year, '.csv'), 
    show_col_types = FALSE
  ) %>% 
  slice_tail(n = 1)

air_swan_count <- read_csv(
  str_c(here::here('output'), '/', analysis_year, '/', 'trumpeter_swans', "_", analysis_year, '.csv'), show_col_types = FALSE
) %>% 
  slice_tail(n = 1) %>% 
  pull(seen_birds_Pop)
```
...
survey_timing <- 
  read_csv(
    str_c(here::here('output'), '/', analysis_year, '/', 'survey_timing', "_", analysis_year, '.csv'), 
    show_col_types = FALSE
  )
```

```{r whooper count, echo = FALSE, warning = FALSE, message = FALSE}
whcr_count <- 
  read_csv(str_c(here::here('output'), '/', analysis_year, '/', 'ws_structured', '.csv')) %>% 
  filter(species == 84 & year == analysis_year) %>%
  mutate(
    count = (pairs * 2) + lonedrake + flockdrake + groups
  ) %>%
...
`r params$year` VCF and population estimate summary statistics for mallards, blue-winged teal, wood ducks, "other ducks", and Canada geese are provided in Table \ref{tab:current_breeding_estimate_table}. Unless otherwise noted, population estimates provided below are derived from state-space models.
...
In `r params$year`, observers counted a total of `r air_coot_count` coots, `r whcr_count` whooping cranes, and `r air_crane_count$cranes_no_groups` sandhill cranes (Figure \ref{fig:annual_crane_counts}). Excluding groups of five or more, `r air_swan_count` trumpeter swans were recorded and the `r params$year` population estimate for swans (accounting for bird density and survey region area, but lacking VCF-correction) was **`r swan_results$mean` (95% CI = `r swan_results$lower`--`r swan_results$upper` individuals)**; Table \ref{tab:trump_tab}; Figure \ref{fig:ssm_trsw}).
...
# acknowledge surveyors
surveyors <- 
  read_csv(here::here('raw_data/archived_survey_data/WSDSobserverCodesMaster.csv')) %>%
...
surveyors <- 
  bind_rows(surveyors, add_surveyors) %>% 
  distinct(name) %>% 
  separate(., col = 'name', c("first", "last"), remove = FALSE, sep = ' ') %>%
  arrange(last)

surveyors <- 
  read_excel(here::here('raw_data/archived_survey_data/Copy of WSDSobserverCodesMaster.xls'), sheet = 1) %>%
  separate(., col = 'Last Name, First Name', c("last_name","first_name"), sep = ',')

surveyors <- 
  read_csv(
    str_c(here::here('raw_data/annual_survey_data'), '/', analysis_year, '_waterfowl.csv'), 
    col_select = c(grd, observer1, observer2), 
    col_types = 'c'
  ) %>%
  filter(grd == '2') %>%
  pivot_longer(
...
We thank Larry Waskow for piloting the aircraft, and Patrice Eyers and Steve Burns for serving as additional aerial observers. For ground surveys, we thank `r surveyors_start`, and `r surveyors_end`. 
